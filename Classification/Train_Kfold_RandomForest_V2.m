...
function [RF_FullModel]=Train_Kfold_RandomForest_V2(Feature,Label,Train,Test,Rand_iter) 

%% ------------------------ written by Maryam Bijanzadeh 09/08/2018 ----------------

% modified --------------- 09/20/2018 -------------
%  difference is that there are some variables that are not outputed to
...
% Xlog = []; Ylog = []; Tlog = []; AUClog=[]; 
    ...
    [Accuracy_Full,ConMat_Full,FValue_RF_Full,PostProbs_Full,NewLabels,NewLabels_Ind,Flag] = Compute_SVM_ONTest(RF_Model,Feature,Label,Train,Test);
...
% RF_FullModel.Label_Test= Label(Test) ; RF_FullModel.Test = Test; RF_FullModel.NewLabelTest= NewLabels_Ind; RF_FullModel.model = RF_Model; 
...
% RF_RandomModel= struct; 
% RF_RandomModel.Accuracy = Accuracy_Rand; RF_RandomModel.ConMat= ConMat_Rand;  RF_RandomModel.FValue = FValue_RF_Rand;RF_RandomModel.Xlog= Xlog_Rand;
% RF_RandomModel.Ylog = Ylog_Rand; RF_RandomModel.AUC = AUClog_Rand;  
% 
